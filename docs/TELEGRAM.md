...
## Оплата (перевод на карту, автоматически)

После оформления заказа бот сам присылает покупателю реквизиты: номер
карты (`PAYMENT_CARD`) и QR-код. Покупатель платит, присылает боту чек —
бот просит подождать проверку и сигналит владельцу в служебный
...
Автоматизация на будущее: Telegram Payments
