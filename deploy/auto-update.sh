...
# Быстрый путь: поменялись ТОЛЬКО файлы в scripts/ (каталог или код импортёра —
# он выполняется через tsx и примонтирован с хоста). Тогда не пересобираем образ
# (это минуты), а просто перезапускаем импорт в работающем контейнере.
DATA_ONLY=1
while IFS= read -r f; do
  [ -z "$f" ] && continue
  case "$f" in
    scripts/*) ;;                      # каталог или импортёр — быстрый путь
    *) DATA_ONLY=0 ;;                   # прочий код (сайт, схема) → пересборка
  esac
done <<< "$CHANGED"
